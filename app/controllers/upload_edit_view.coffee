...
class UploadEditView extends Spine.Controller
  ...
  elements:
    ...
    '.fileupload-buttonbar'       : 'buttonBarEl'
    '#fileupload'                 : 'formEl'
  ...
  alldone: (e, data) ->
    @log e, data
    @log @formEl
    @log @formEl.data('blueimp-fileupload')
    @log @formEl.data('fileupload')
    f = @formEl.data('blueimp-fileupload')
    @formEl.trigger('destroy')
